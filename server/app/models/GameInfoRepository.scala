...
class GameInfoRepository(db: Database)(implicit val executionContext: ExecutionContext) {
  ...
  def getGameInfo(gameId: Int): Future[Option[GameInfo]] = {
    val dbQuery = (for {
      game <- Tables.GameInfo if game.id === gameId
      gamePlayers <- Tables.GamePlayers.filter(_.gameId === game.id)
    } yield (game, gamePlayers)).result
    db.run(dbQuery).map(_.groupBy(_._1).headOption.map(r => dbToGameInfo(r._1, r._2.map(_._2))))
  }

  def joinGame(username: Username, gameId: Int): Future[Boolean] = {
    getGameInfo(gameId).map(_.exists { gameInfo =>
      gameInfo.status == GameStatus.WaitingToStart && gameInfo.playerCount > gameInfo.players.size
    }).flatMap {
      case true =>
        db.run(Tables.GamePlayers.insertOrUpdate(GamePlayersRow(gameId, username.value))).map(_ > 0)
      case _ =>
